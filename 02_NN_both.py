...
X_train_full = np.load(path + 'X_train_both.npy')

y_train_full = np.load(path + 'y_train_both.npy')
...
history_weight = model.fit(X_train_full, y_train_cat, epochs=20, validation_split = 0.1)

model.save(save_path + 'model_trained_both')
np.save(save_path + 'loss_both', history_weight.history['loss'])
np.save(save_path + 'val_loss_both', history_weight.history['val_loss'])
